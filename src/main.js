...
async function saveProfile(event){event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget));values.email_updates=event.currentTarget.email_updates.checked;values.photo=state.photoDraft;const button=event.currentTarget.querySelector('[type=submit]'),label=button.textContent;button.disabled=true;button.textContent='Saving profile…';try{await api(`/api/employees/${state.selected}/profile`,{method:'PATCH',body:JSON.stringify(values)});await refreshBootstrap();state.detail=await api('/api/employees/'+encodeURIComponent(state.selected));state.photoDraft=state.detail.employee.photo||'';state.page='profile';render();toast('Profile saved.')}catch(e){toast(e.message,'bad');button.disabled=false;button.textContent=label}}
